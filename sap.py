...
def cell_change(r,c,button):
    if a[r][c].mode == 'closed':
        if button == 1:
            ...
            global bombs_count
            global num_bombs
            if opened == (120 - bombs_count):
                    counts = bombs_count * 10
                    reit = open("reiting.txt","r")
                    txt = reit.read()
                    counts_other = list(map(int,(str(txt).split(" "))))
                    reit.close()
                    counts_other.append(counts)
                    counts_other.sort()
                    pos = len(counts_other) - counts_other.index(counts)
                    print('You win!!! Your counts: ',counts,'. Your position in reiting: ',pos, 'Try play again!')
                    wrt = open("reiting.txt","w")
                    wrt.write(txt + " " + str(counts))
                    wrt.close()
                    time.sleep(3)
                    num_bombs  = 0
                    opened = 0
